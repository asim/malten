...
func TestRestartPreservesCaptureAndModeration(t *testing.T) {
	...
	if strings.Contains(w.Body.String(), p.ID) {
		t.Fatal("quarantined post exposed")
	}
	w = request(t, restarted, "POST", "/api/posts/one/delete", "", token)
	if w.Code != 204 {
		t.Fatal("owner cannot delete after restart", w.Code)
	}
	again, err := Open(path)
	if err != nil || len(again.stream.posts) != 0 {
		t.Fatal("deleted post returned", err)
	}
	raw, _ := os.ReadFile(path)
	if bytes.Contains(raw, []byte("data:image")) {
		t.Fatal("deleted media retained")
	}
	info, _ := os.Stat(path)
	if info.Mode().Perm() != 0600 {
		t.Fatal("snapshot is not private")
	}
}
...
func TestRestartPurgesExpiredMedia(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(filepath.Dir(path), ".stream.json-abandoned"), []byte("partial media"), 0600); err != nil {
		t.Fatal(err)
	}
	restarted, err := Open(path)
	if err != nil || len(restarted.stream.posts) != 0 {
		t.Fatal("expired post restored", err)
	}
	raw, _ := os.ReadFile(path)
	if bytes.Contains(raw, []byte("expired-media")) {
		t.Fatal("expired media retained on disk")
	}
	matches, _ := filepath.Glob(filepath.Join(filepath.Dir(path), ".stream.json-*"))
	if len(matches) != 0 {
		t.Fatal("abandoned snapshot retained")
	}
}
...
func TestStorageFailureDoesNotAcknowledgeCapture(t *testing.T) {
	...
	if err := s.stream.publish(context.Background(), Post{Text: "draft"}); err == nil {
		t.Fatal("uncommitted capture acknowledged")
	}
	if len(s.stream.posts) != 0 {
		t.Fatal("failed capture exposed")
	}
	restarted, err := Open(path)
	if err != nil || len(restarted.stream.posts) != 0 {
		t.Fatal("failed capture restored", err)
	}
}
...
func TestCorruptSnapshotIsNotOverwritten(t *testing.T) {
	...
	if _, err := Open(path); err == nil {
		t.Fatal("silently ignored corrupt snapshot")
	}
	raw, _ := os.ReadFile(path)
...
func TestAcceptedPostAndReviewSurviveRestart(t *testing.T) {
	...
	restarted, err := Open(path)
	if err != nil || len(restarted.stream.posts) != 1 {
		t.Fatal("acknowledged capture lost", err)
	}
	...
	restarted, err = Open(path)
	if err != nil || !restarted.stream.posts[0].hidden {
	...
	restarted, err = Open(path)
	if err != nil || len(restarted.stream.posts) != 0 {
		t.Fatal("rejected capture restored", err)
	}
	var saved snapshot
	raw, _ := os.ReadFile(path)
	if err := json.Unmarshal(raw, &saved); err != nil || len(saved.Posts) != 0 {
		t.Fatal("rejected data on disk", err)
	}
}
...
func TestAgentDoesNotRepeatLiveCaptureAfterRestart(t *testing.T) {
	...
	if err := s.PublishAgent(context.Background(), "reminder", "reflection", "Reminder"); err != nil {
		t.Fatal(err)
	}
	...
	if len(s.stream.posts) != 1 || s.stream.posts[0].Created != created {
		t.Fatal("agent repeated or renewed live capture")
	}
}
