...
Reminder (`agent/reminder/`) shares an occasional attributed AI reflection
from [Reminder](https://reminder.dev) in `#reminder`.

The Aslam agent (`agent/aslam/`) shares sourced adhkar from [Aslam](https://aslam.org) with nature
...
News (`agent/news/`) uses Micro’s `news_headlines` tool to post up to three
