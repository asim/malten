...
(async()=>{
 ...
 elements.get('thought').value='x'.repeat(20001);await elements.get('composer').onsubmit({preventDefault(){}});
 assert.equal(elements.get('thought').value.length,20001,'over-limit text is kept rather than truncated');
 assert(elements.get('status').textContent.includes('20,000'));
 assert(!source.includes('maxlength="1200"'),'typing is not silently capped');
 ...
 assert(![...pending.values()].some(p=>p.kind==='summary'),'accepted summary request leaves local outbox');
 offline=true;await elements.get('summarise').onclick();
 assert([...pending.values()].some(p=>p.kind==='summary'),'offline summary request is durable');
 window.location.hash='#another';listeners.hashchange();await new Promise(setImmediate);
 offline=false;listeners.online();await new Promise(setImmediate);
 assert.equal(summarySent.stream,random,'reconnect sends request to original stream');
 assert(![...pending.values()].some(p=>p.kind==='summary'),'reconnected summary leaves outbox');
 ...
 assert.equal(elements.get('summarise').hidden,true,'no summary action without captures');
